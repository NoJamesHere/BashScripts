...
echo; echo -e "${HEADER} How to use \${}.${RESET}"   # --- ${} ---
                                                     #
name=Anonymous                                       # ${} : For starters, it's needed for color ansi escape codes. otherwise it literally just prints `\033[whatever;1m`.
...
((numB++))                                                   # $(() : On (tl;1) it increments B by 1, causing it to be
echo; echo "B++ ; B:${numB}"                                 # $(() : 3 now instead of the previous 2. (easy, right?)
